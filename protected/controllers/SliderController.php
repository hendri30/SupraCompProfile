		...
		if(isset($_POST['Slider']))
		{
			...
			if(!empty($file)){
				if($model->save()){
					...
					$notif->setFlash('success',"<strong>Create Slider Success!</strong>");
				}
				$this->redirect(array('view','id'=>$model->id));
			}else{
				$notif=Yii::app()->getComponent('user');
				$notif->setFlash('error',"<strong>Create Slider Failed!</strong>");
			}
		}
			...
			if(file_exists(Yii::app()->basePath.self::URLUPLOAD.$model->file_name)){
				...
				if(empty($cekfile)){
					...
					if($model->save()){
						...
						$notif=Yii::app()->getComponent('user');
						$notif->setFlash('error',"<strong>Update Slider Failed!</strong>");
					}
				}
				else{
					$model->attributes=$_POST['Slider'];
